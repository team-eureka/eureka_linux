...
memory_node_t *memory_engine_lookup_shm_node_for_size(
	struct rb_root *shm_root, size_t phyaddress)
{
	struct rb_node *n = shm_root->rb_node;
	memory_node_t *tmp_node;
								...
								size_t alignment, memory_node_t **node)
{
	...
	new_node->m_reference_count = 1;
	INIT_LIST_HEAD(&new_node->m_user_list);
	new_node->m_task_free_flag = 1;

	new_node->m_phyaddress = MEMNODE_ALIGN_ADDR(new_node);
	memory_engine_insert_shm_node(&(engine->m_shm_root), new_node);
	new_node->b_takeover = 0;
...
int memory_engine_free(memory_engine_t *engine, int alignaddr)
{
	...
	if (node == NULL) {
		printk("memory free again physicaladdress[%x] threadid[%d]"
			"threadname[%s]\n", alignaddr, task_pid_vnr(current),
			current->comm);
		res = -EFAULT;
		goto err_exit;
	}
	...
	if (node->b_takeover == 1) {
		...
		node->m_task_free_flag = 0;
		goto FREE;
	}

	taskid = task_tgid_vnr(current);
	if (node->m_taskid != taskid) {
		list_for_each_entry_safe(tmp, q, &node->m_user_list, m_list) {
				...
				shm_driver_operation_t op)
{
	...
	char *ptr_tag;
	if (engine == NULL || op.m_param1 == 0
		|| op.m_param2 == 0 || op.m_param3 == 0) {
		return -EINVAL;
	}

	if (shm_lowmem_debug_level > 2) {
		mutex_lock(&(engine->m_mutex));
		node = memory_engine_lookup_shm_node_for_cache(
			&(engine->m_shm_root), op.m_param3, op.m_param2);
		mutex_unlock(&(engine->m_mutex));
		if ((node == NULL) || (node->m_next_free != NULL)) {
			...
			shm_error("%s %s cache Error, Can't found "
				"viraddr[0x%lx] phyaddr[0x%lx] "
				"Size[0x%lx] memory node\n",
				__FUNCTION__, ptr_tag, op.m_param1,
...
int memory_engine_show(memory_engine_t *engine, struct seq_file *file)
{
	...
	len += seq_printf(file, "  No| Alloc Mem Ref|   Offset  |    Addr   |     Size  |    Align  "
		"| thread id(name)[free] pid(name)[takeover][refcount]\n");
	len += seq_printf(file, "---------------------------------------------------------------"
		"----------------------------------------------\n");
	...
	for (new_node = engine->m_root.m_next; new_node->m_size != 0;
		...
		len += seq_printf(file, " %3d|%6s[%1d][%3d]| 0x%8X| 0x%8x| 0x%8x| 0x%8x"
			"| %4d(%16s)[%1d] %4d(%16s)[%1d][%d]",
			++i, ptr_tag, new_node->memory_type, new_node->m_reference_count,
			(MEMNODE_ALIGN_ADDR(new_node) - engine->m_base),
			new_node->m_phyaddress, new_node->m_size, new_node->m_alignment,
			new_node->m_threadid, new_node->m_threadname, new_node->m_task_free_flag,
			new_node->m_taskid, new_node->m_taskname, new_node->b_takeover,
			new_node->m_refcount);

		if (new_node->m_next_free == NULL) {
			list_for_each_entry(tmp, &new_node->m_user_list, m_list) {
				len += seq_printf(file, "| %5d[%16s][%d]", tmp->m_threadid,
					tmp->m_threadname, tmp->m_refcount);
			}
		}
		len += seq_printf(file, "\n");
	}
	...
	shm_debug("memory_engine_show OK. (node = %d, len = %d)\n", i, len);
 ...
 */
int memory_engine_release_by_taskid(shm_device_t *device, pid_t taskid)
{
	size_t alignaddr = 0 ;
	memory_node_t *new_node = NULL;
	...
	for (new_node = engine->m_root.m_next ; new_node->m_size != 0 ; new_node = new_node->m_next) {
		...
		if ((taskid == new_node->m_taskid) && (!new_node->b_takeover)) {
		...
		* than swift to dynamic allocate array */
		if (node_max == rlsCnt) {
			...
			dynamic_release_array = kzalloc(i * 1024 * sizeof(int), GFP_KERNEL);
	...
	for (idx = 0 ; idx < rlsCnt ; idx++) {
		...
		MV_SHM_iounmap_and_free_rbtree(device, alignaddr - device->m_base);
